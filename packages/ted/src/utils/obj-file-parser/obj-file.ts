/**
 * Source: https://github.com/WesUnwin/obj-file-parser
 *
...
export default class OBJFile {
  ...
  parse() {
    ...
    const _stripComments = (lineString: string) => {
  ...
  _parseVertexCoords(lineItems: string[]) {
  ...
  _parseTextureCoords(lineItems: string[]) {
  ...
  _parseVertexNormal(lineItems: string[]) {
    const x = lineItems.length >= 2 ? parseFloat(lineItems[1]) : 0.0;
    const y = lineItems.length >= 3 ? parseFloat(lineItems[2]) : 0.0;
    const z = lineItems.length >= 4 ? parseFloat(lineItems[3]) : 0.0;
  ...
  _parseMtlLib(lineItems: string[]) {
    if (lineItems.length >= 2) {
      this.result.materialLibraries.push(lineItems[1]);
    }
  }

  _parseUseMtl(lineItems: string[]) {
    if (lineItems.length >= 2) {
